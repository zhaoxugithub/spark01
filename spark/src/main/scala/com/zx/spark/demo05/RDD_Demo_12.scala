...
object RDD_Demo_12 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    println("##########reduceByKey###########")
    val value: RDD[(String, Int)] = dataRDD1.reduceByKey(_ + _)
    ...
    dataRDD1.combineByKeyWithClassTag(num=>num,(x:Int,y:Int)=>{x+y},(x:Int,y:Int)=>{x+y}).foreach(println)
     ...
     */
    dataRDD1.combineByKey(num=>num,(x:Int,y:Int)=>{x+y},(x:Int,y:Int)=>{x+y}).foreach(println)
     ...
     */
    //相同key求和
    dataRDD1.aggregateByKey((0))((x:Int,y:Int)=>{x+y},(x:Int,y:Int)=>{x+y}).foreach(println)
    //相同key求平均
    /*
     ...
     */
//    dataRDD1.aggregateByKey((0,0))((x,y)=>(x._1+y,x._2+1),(t1,t2)=>(t1._1+t2._1,t1._2+t2._2)).map(x=>x._1/x._2)foreach(println)
